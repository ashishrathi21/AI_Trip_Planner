...
export const createTripWithAI = async (req, res) => {
  try {
    ...
    const { destination, days, budget, interests, travelstyle, travelersType } = req.body;
...
export const deleteTripById = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Trip deleted permanently from everywhere" });
    } catch (error) {
